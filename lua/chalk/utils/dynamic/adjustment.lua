...
function M.apply_color_change(group_name, new_color, color_var_name)
	...
	if not new_color or not new_color:match("^#%x%x%x%x%x%x$") then
		shared.notify(string.format("Invalid color format: %s", new_color or "nil"), vim.log.levels.ERROR, "Dynamic")
		return
	end
...
function M.increase_brightness(group_name, current_color)
	...
	local new_color = shared.color_utils().adjust_brightness(current_color, step, M.MIN_BRIGHTNESS, M.MAX_BRIGHTNESS)
...
function M.reset_group_color(group_name)
	...
	shared.notify(string.format("Reset color for %s", group_name), vim.log.levels.INFO, "Dynamic")
end
